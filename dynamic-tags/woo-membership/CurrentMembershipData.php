    ...
    public function render() {
        ...
        switch ($settings['data_type']) {
            ...
            case 'member_since':
                ...
                echo $start_date ? '<time datetime="' . date('Y-m-d', $start_date) . '" title="' . esc_attr(date_i18n(wc_date_format(), $start_date)) . '">' . date_i18n($date_format, $start_date) . '</time>' : esc_html__('N/A', 'woocommerce-memberships');
                break;
            case 'next_bill_on':
                if ($has_subscription) {
            ...
            case 'remaining_time':
                ...
                if ($expiry_date && $expiry_date > $current_time) {
                    ...
                    echo str_replace('{remaining_time}', $remaining, $settings['remaining_time_text']);
                } elseif ($expiry_date && $expiry_date <= $current_time) {
                    echo $settings['expired_text'];
                } else {
                    echo $settings['no_expiry_text'];
                }
                break;
